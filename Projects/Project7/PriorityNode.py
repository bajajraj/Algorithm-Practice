...
class MaxNode(PriorityNode):
    ...
    def __lt__(self, other: 'MaxNode') -> bool:
        """
        Less than comparator. Determines if self is greater than
        other node, and thus should be sorted lower than other node in
        priority queue.

        :param other: node to compare self to
        :return: bool - False if self is less than other,
                 True if self is greater than other
        """
        if self.priority > other.priority:
            return True
        if self.priority == other.priority and self.value > other.value:
            return True
        return False

    def __gt__(self, other: 'MaxNode') -> bool:
        """
        Greater than comparator. Determines if self node is less than other node,
        and thus should be sorted higher than other node in priority queue.

        :param other: node to compare self to
        :return: bool - False if self is greater than other,
...
class MinNode(PriorityNode):
    ...
    def __lt__(self, other: 'MinNode') -> bool:
        """
        Less than comparator. Determines if self node is less than other node,
        and thus should be sorted higher than other node in priority queue.

        :param other:  node to compare self to
        :return: bool - True if self is less than other,
                 False is self is greater than other
        """
        if self.priority < other.priority:
            return True
        if self.priority == other.priority and self.value < other.value:
    ...
    def __gt__(self, other: 'MinNode') -> bool:
        """
        Greater than comparator. Determines if self node is greater than other node,
        and thus should be sorted lower than other node in priority queue.

        :param other: node to compare self to
        :return: bool - True if self is greater than other,
                 False if other is less than other
        """
        if self.priority > other.priority:
            return True
        if self.priority == other.priority and self.value > other.value:
